...
class Chat(Frame):
    ...
    def __init__(self, master=None):
        ...
        self.tabTopBar.grid(row=0, column=0, columnspan=4, sticky="nsew")
    ...
    def createWidgets(self):
        ...
        self.nickname_Button = Button(self.tabTopBar, text="Names", command=lambda: self.openNameWindow(), bg="#25D366",  activebackground="#21B858", font=('HelveticaNeue', 11))
    ...
    def openNameWindow(self):
        ...
        Label(newWindow, text="Enter an Unwanted Nickname:", font=('HelveticaNeue', 11)).grid(row=7, column=0, padx=10, pady=10)

        unwantedButton = Button(newWindow, text=" add Name ",
                                  command=lambda: [self.addUnwanted(entryFieldUnwanted.get()),entryFieldUnwanted.delete(0,'end')], bg="#34B7F1",
                                  activebackground="#0f9bd7", font=('HelveticaNeue', 11))
    ...
    def changeFriendsUsername(self, newFriendsUsername, oldName):
        ...
        # check whether nickname is already in use:
        nameAlreadyInUse = False
        values = file.values()
        for name in values:
            if name == newFriendsUsername:
                nameAlreadyInUse = True
                break
        if nameAlreadyInUse == True:
            print("This nickname is already in use. Please use an other one.")
            newWindow = Tk()
            newWindow.title("Nickname already in use")
            self.geometry = newWindow.geometry("300x100")
            Label(newWindow, text="This nickname is already in use.\nPlease use an other one", font=('HelveticaNeue', 11)).grid(row=2, column=0,
                                                                                               padx=10, pady=10)

        else:
            # create an event
            changeFriendsUsernameEvent = self.ecf.next_event('chat/nameChanged', {'newName': newFriendsUsername, 'fromUser': self.username, 'oldFriendsUsername': oldName})
            chat_function = ChatFunction()
